...
/*streznik.get("/", (req,odgovor) => {
    ...
    odgovor.sendFile(path.resolve(__dirname, "index.html"));
});*/
...
streznik.get("/*", (req, odgovor) => {
    odgovor.sendFile(path.resolve(__dirname, req.originalUrl));
});
...
async function detailedScan(host, odgovor) {
    var osandports = new nmap.NmapScan(host, '-sS --max-retries 1 -T5');
