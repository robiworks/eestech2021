...
async function constructLocal(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        let o = row.insertCell();
        //ocena = 
        ocena = Math.round(Math.log(ocena)*10)/10;
        o.style="text-align:center; cursor:pointer";
        o.innerHTML = '<span class="badge rounded-pill ' + types[(ocena < 3) ? Math.round(ocena)-1 : 2] + '">' + ocena + '</span> ';
...
async function showPopup(e) {
    ...
    mod_body.innerHTML += "Ocena ranjivosti je enaka logaritmu vsote stopnje ranljivosti.";
}
...
async function awaitUpdate() {
    ...
    let userValue = parseInt(document.getElementById('updateInterval').value);
    if (userValue < value) {
        value = userValue;
    }
    if (value > 0 ) {
...
async function callServer() {
    ...
    await fetch("http://127.0.0.1:8080/api",requestOptions)
